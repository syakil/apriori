...
class ProsesAprioriController extends Controller
{
    public function proses(){
        ...
        $support_item1 = DB::select('UPDATE `itemset1` SET support = jumlah/'.$jumlah_transaksi .'*100,lolos = if(support > jumlah,1,0) ');
        $lolos_item1 = DB::select('UPDATE `itemset1` SET lolos = if(support > jumlah,1,0) ');
    }
}
